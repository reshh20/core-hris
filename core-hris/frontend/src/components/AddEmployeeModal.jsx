...
import { createEmployee, fetchDepartments, fetchPositions, fetchEmployees } from '../services/employeeService';
...
export default function AddEmployeeModal({ isOpen, onClose, onSuccess }) {
  ...
  const validate = () => {
    ...
    else if (!/^EMP\d{3,}$/.test(form.employee_id)) errors.employee_id = 'Format: EMP followed by 3+ digits (e.g., EMP021)';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || err.response?.data?.message || 'Failed to create employee';
  ...
  const inputClass = (field) =>
    `w-full px-3 py-2.5 rounded-lg border text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500/30 ${
      fieldErrors[field]
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
        <div className="sticky top-0 bg-white border-b border-surface-200 px-6 py-4 rounded-t-2xl flex items-center justify-between z-10">
          <h2 className="text-lg font-bold text-surface-900">Add New Employee</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
            <div>
              ...
              <input name="employee_id" value={form.employee_id} onChange={handleChange} placeholder="EMP021" className={inputClass('employee_id')} />
              {fieldErrors.employee_id && <p className="text-xs text-red-500 mt-1">{fieldErrors.employee_id}</p>}
            </div>

            <div>
              <label className={labelClass}>Email *</label>
              <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="john.doe@corehris.com" className={inputClass('email')} />
              {fieldErrors.email && <p className="text-xs text-red-500 mt-1">{fieldErrors.email}</p>}
            </div>

            <div>
              <label className={labelClass}>First Name *</label>
              <input name="first_name" value={form.first_name} onChange={handleChange} placeholder="John" className={inputClass('first_name')} />
              {fieldErrors.first_name && <p className="text-xs text-red-500 mt-1">{fieldErrors.first_name}</p>}
            </div>

            <div>
              <label className={labelClass}>Last Name *</label>
              <input name="last_name" value={form.last_name} onChange={handleChange} placeholder="Doe" className={inputClass('last_name')} />
              {fieldErrors.last_name && <p className="text-xs text-red-500 mt-1">{fieldErrors.last_name}</p>}
            </div>

            <div>
              <label className={labelClass}>Phone *</label>
              <input name="phone" value={form.phone} onChange={handleChange} placeholder="+91 9876543230" className={inputClass('phone')} />
              {fieldErrors.phone && <p className="text-xs text-red-500 mt-1">{fieldErrors.phone}</p>}
            </div>

            <div>
              <label className={labelClass}>Location *</label>
              <input name="location" value={form.location} onChange={handleChange} placeholder="Mumbai, India" className={inputClass('location')} />
            ...
            <div>
              ...
              {fieldErrors.employment_status && <p className="text-xs text-red-500 mt-1">{fieldErrors.employment_status}</p>}
            </div>
            ...
            <div>
              ...
              {fieldErrors.profile_image && <p className="text-xs text-red-500 mt-1">{fieldErrors.profile_image}</p>}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 mt-8 pt-5 border-t border-surface-200">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-lg text-sm font-medium text-surface-600 hover:bg-surface-100 transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="px-5 py-2.5 rounded-lg text-sm font-semibold text-white bg-gradient-to-r from-primary-500 to-primary-700 hover:from-primary-600 hover:to-primary-800 shadow-lg shadow-primary-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
